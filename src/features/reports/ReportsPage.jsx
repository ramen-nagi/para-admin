...
function ReportDetails({ report, onClose, onUpdated }) {
  ...
  async function handleSave(event) {
    ...
    const resolvedAt = status === 'resolved'
      ? (report.status === 'resolved' && report.resolved_at ? report.resolved_at : new Date().toISOString())
      : null
    const result = await updateReport(report.id, { status, adminNotes, resolvedAt })
  ...
  return (
    <div className="detail-overlay" role="presentation" onClick={onClose}>
      <section className="detail-panel" role="dialog" aria-modal="true" aria-labelledby="report-detail-title" onClick={(event) => event.stopPropagation()}>
        <div className="detail-header">
          <div><p className="eyebrow">Report details</p><h2 id="report-detail-title">{categoryLabels[report.category] ?? report.category}</h2></div>
          <button className="close-button" type="button" aria-label="Close report details" onClick={onClose}>×</button>
        </div>
        <div className="detail-status"><span className={`status-badge ${report.status}`}>{statusLabels[report.status] ?? report.status}</span><span>{formatDate(report.created_at)}</span></div>
        <div className="detail-section"><h3>Description</h3><p className="report-description">{report.description}</p></div>
        ...
        <form className="edit-section" onSubmit={handleSave}>
          <div className="edit-field"><label htmlFor="report-status">Status</label><select id="report-status" value={status} onChange={(event) => setStatus(event.target.value)}>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
          <div className="edit-field"><label htmlFor="admin-notes">Admin notes</label><textarea id="admin-notes" rows="5" value={adminNotes} onChange={(event) => setAdminNotes(event.target.value)} placeholder="Add internal notes about this report…" /></div>
          {saveError && <p className="error-message" role="alert">{saveError}</p>}
          {saved && <p className="success-message" role="status">Report updated successfully.</p>}
...
function ReportsPage({ userEmail, onSignOut }) {
  ...
  const loadReports = useCallback(async () => {
    ...
    if (result.error) setError('Reports could not be loaded. Check your connection and permissions.')
