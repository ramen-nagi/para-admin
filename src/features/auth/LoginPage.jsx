...
function LoginPage({ onSignedIn }) {
  ...
  async function handleSubmit(event) {
    ...
    setSubmitting(true)
    const { session: authenticatedSession, error: signInError, isAdmin } =
  ...
  if (loading) {
    return <AuthLayout><p className="loading">Loading…</p></AuthLayout>
  }
      ...
      <section className="auth-card" aria-labelledby="signin-title">
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="field-group">
            ...
            <input id="password" name="password" type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Enter your password" required />
          </div>
