...
test('admins can open operations, GTFS, and account management', () => {
  for (const tab of [...OPERATION_TABS, 'gtfs', 'accounts', 'passengers', 'staff', 'my-account'])
...
test('editors can open GTFS and My Account only', () => {
  ...
  for (const tab of [...OPERATION_TABS, 'accounts', 'passengers', 'staff'])
...
test('operators can open operations and My Account but not restricted tools', () => {
  ...
  for (const tab of ['gtfs', 'accounts', 'passengers', 'staff'])
    assert.equal(canAccessTab('operator', tab), false)
})
